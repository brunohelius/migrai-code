...
func (s *PersistentShell) execCommand(command string, timeout time.Duration, ctx context.Context) commandResult {
	...
	tempDir := os.TempDir()
	stdoutFile := filepath.Join(tempDir, fmt.Sprintf("migrai-code-stdout-%d", time.Now().UnixNano()))
	stderrFile := filepath.Join(tempDir, fmt.Sprintf("migrai-code-stderr-%d", time.Now().UnixNano()))
	statusFile := filepath.Join(tempDir, fmt.Sprintf("migrai-code-status-%d", time.Now().UnixNano()))
	cwdFile := filepath.Join(tempDir, fmt.Sprintf("migrai-code-cwd-%d", time.Now().UnixNano()))
